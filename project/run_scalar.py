...
class Linear(minitorch.Module):
    ...
    def forward(self, inputs):
        outputs = []
        for j in range(len(self.bias)):  # For each output node
            weighted_sum = sum(self.weights[i][j].value * inputs[i] for i in range(len(inputs)))
            output = weighted_sum + self.bias[j].value  # Linear transformation
